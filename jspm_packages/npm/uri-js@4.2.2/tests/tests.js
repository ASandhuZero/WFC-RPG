...
(function(process) {
  ...
  test("Convert URI to IRI", function() {
    ...
    strictEqual(URI.serialize(URI.parse("uri://xn--99zt52a.example.org/%e2%80%ae"), IRI_OPTION), "uri://xn--99zt52a.example.org/%E2%80%AE");
  ...
  if (URI.SCHEMES["mailto"]) {
    test("Mailto Parse", function() {
      ...
      components = URI.parse("mailto:joe@example.com?cc=bob@example.com?body=hello");
      if (URI.VALIDATE_SUPPORT)
        ok(components.error, "invalid header fields");
      ...
      deepEqual(components.to, ["user@example.org"], "to");
      strictEqual(components.subject, "=?utf-8?Q?caf=C3=A9?=", "subject");
      components = URI.parse("mailto:user@example.org?subject=%3D%3Fiso-8859-1%3FQ%3Fcaf%3DE9%3F%3D");
      deepEqual(components.to, ["user@example.org"], "to");
      strictEqual(components.subject, "=?iso-8859-1?Q?caf=E9?=", "subject");
