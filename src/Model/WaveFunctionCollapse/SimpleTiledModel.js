...
export function SimpleTiledModel(periodic, width, height, tileset_info) {
    ...
    while (result == null) {
        ...
        if (result) {
            return GenerateTileMap(wave, tile_amount, subset["tiles"].names, width, height)
        }
 ...
 */
function MaxTiles(subsets) {    let entries = Object.entries(subsets)
    let tile_amount = 0;
        ...
        for (let d = 0; d < 4; d++) {
            ...
            for (let l = 0; l < p.length; l++) {
                ...
                if (comp[d] == 0) {
                    ...
                    // BanItem(wave, subset["items"], index_2, 1);
                }
            }
        }
    }
...
// export class SimpleTiledModelClass extends Model {
//     constructor(periodic, width, height, tileset_info) {
//         super(width, height);
...
//             let subset = {}
//             let subset_info = subsets_info[i];
//             this.subset_names.push(subset_info["name"])

//             let tiles_info = subset_info["tiles_info"];
//             let items_info = subset_info["items_info"];
//             let neighbors_info = subset_info["neighbors"]
//             let rules_info = subset_info["rules"]
//             let tiles, items, neighbors, rules;
    
//             tiles = Constraints.GenerateTiles(tiles_info);
...
//                 }
//             }
//             if (rules_info.length > 0) {
//                 rules = Constraints.GetItemRules(rules_info);
//             }
//             subset["items"] = items;
//             subset["items"]["rules"] = rules;
//             subset["tiles"] = tiles
...
//             this.subsets[subset_info["name"]] = subset
