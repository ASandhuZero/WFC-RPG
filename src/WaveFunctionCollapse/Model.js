export class Model {
    ...
    Init() {
        ...
        for (let i = 0; i < init_array_length; i++) {
            this.wave[i] = new Array(this.tiles.length);
        ...
        this.summed_weights = new Array(init_array_length);
        this.summed_log_weights = new Array(init_array_length);
    ...
    Propagate() {
        ...
        while(this.stacksize > 0) {
            ...
            for (let d = 0; d < 4; d++) {
                ...
                let p = this.propagtor[d][e1.Item2];
    ...
    Ban(i, t) { // i is item, t is tile
        ...
        for (let d = 0; d < 4; d++) {
            ...
            let sum = this.sums_of_weights[i];
            this.entropies[i] += this.sums_of_log_weights[i] / sum - Math.log(sum);

            this.sums_of_ones[i] -= 1;
            this.sums_of_weights[i] -= this.weights[t];
            this.sums_of_log_weights[i] -= this.log_weights[t];

            sum = sums_of_weights[i];
            this.entropies[i] -= this.sums_of_log_weights[i] / sum - Math.log(sum);
        }
    }
